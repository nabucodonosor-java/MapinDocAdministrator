            ...
            <div className="hp-details-cards">
                {isLoading ? <InfoLoader /> : (
                    <div className="base-card border-radius-20 hp-details-content-img mr-3">
                        ...
                        <div className="base-card hp-details-places">   
                            ...
                            name={hp?.placeService.name === undefined ? 'sem nome' : hp.placeService.name}
                            logradouro={hp?.placeService.logradouro === undefined ? 'sem endereço' : hp.placeService.logradouro}
                            complemento={hp?.placeService.complemento === undefined ? 'sem complemento' : hp.placeService.complemento}
                            localidade={hp?.placeService.localidade === undefined ? 'sem cidade' : hp.placeService.localidade}
                            uf={hp?.placeService.uf === undefined ? 'sem uf' : hp.placeService.uf}
                            />
                        </div>
                    </div>
                )}    
                ...
                <div className="base-card border-radius-20 hp-details-content-info">
                    <div className="hp-details-info">
                        <h4>Informações</h4>
                        <span><strong>CRM/CREDITO: </strong>{hp?.register}</span>
                    
                        <span><strong>NOME: </strong>{hp?.name}</span>
             
                        <span><strong>CELULAR: </strong>{hp?.phone == undefined ? 'sem celular cadastrado' : hp.phone}</span>         
                        
                        <span><strong>EMAIL: </strong>{hp?.email == undefined ? 'sem e-mail cadastrado' : hp.email}</span>      
                        <h4>Currículo</h4>
